 ...
 * Every step is documented in detail for educational purposes.
 */
...
void Heap::repair_up(int index) {
    ...
    if ((data_array.size() > 1) && (index > 0) && (index <= data_array.size() - 1)) {
        ...
        // is repaired (the father must have an equal or higher value than the vertex repaired. If not, they have to
        // switch places.
        while (data_array[father] < data_array[index]) {
            // This will swap the values stored on the two addresses within the vector and is similar to calling
            // swap(*vertex_a_pointer, *vertex_b_pointer).
            iter_swap(data_array.begin() + index, data_array.begin() + father);
            // Now as the value was assigned to the father vertex we have to update the index to the index of the father
            // vertex.
            index = father;
...
void Heap::repair_down(int index) {
    ...
    if ((data_array.size() > 1) && (index >= 0) && (index < data_array.size() - 1)) {
        ...
        const int last_possible_index = data_array.size() - 1;
        ...
        while ((data_array[index] < data_array[child_index_a]) || (data_array[index] < data_array[child_index_b])) {
            if (data_array[child_index_a] > data_array[child_index_b]) {
                iter_swap(data_array.begin() + index, data_array.begin() + child_index_a);
                index = child_index_a;
            } else {
                iter_swap(data_array.begin() + index, data_array.begin() + child_index_b);
                index = child_index_b;
            }
...
// Constructor
Heap::Heap(const vector<int>& start_vector) : data_array(start_vector) {}

vector<int> Heap::read() const { return data_array; }
...
void Heap::remove(const int index) {
    // we remove by a similar technique as delete_max by changing the priority of the last Element and deleting the last
    // priority.
...
int Heap::change_priority(const int index, const int new_priority) {
    // If the priority of one vertex changed we have to update the current priority to the new priority and repair the
    // Heap Order.
    const int old_priority = data_array[index];
...
void Heap::build_heap() {
    // To build the Heap from scratch in case an array was assigned to the Heap, we start at the last vertex that is not
    // a Leaf then we call repair_down on each vertex as with each level of depth that was repaired the Heap Order is
    // restored for this level and all levels below.
